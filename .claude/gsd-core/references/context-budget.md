...
The keys live in `.claude/settings.json` (project) or `E:/externalProjects/workforce-management/.claude/settings.json` (global) — **not** in `.planning/config.json`:
...
See @E:/externalProjects/workforce-management/.claude/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
